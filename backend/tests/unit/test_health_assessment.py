...
class TestHealthAssessmentService:
    ...
    @pytest.mark.unit
    def test_assessment_response_structure(self, mock_ai_assessment):
    ...
    def test_symptom_severity_validation(self):
        ...
        from tests.test_helpers import validate_symptom_severity

        # Valid severity
        assert validate_symptom_severity({"headache": 7}) is True

        # Invalid severity (too high)
        assert validate_symptom_severity({"headache": 15}) is False

        # Invalid severity (too low)
        assert validate_symptom_severity({"headache": 0}) is False
