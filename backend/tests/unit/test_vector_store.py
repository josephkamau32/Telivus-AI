...
class TestVectorStoreService:
    ...
    async def test_similarity_search_returns_results(
        ...
        with patch(
            'app.services.vector_store_simple.simple_vector_store.search_documents',
    ...
    async def test_embedding_generation(self):
        ...
        assert len(embedding) == 384  # Our simple embedding dimension
    ...
    async def test_medical_context_building(self):
        ...
        with patch(
            'app.services.vector_store_simple.search_medical_knowledge',
            new_callable=AsyncMock,
        ) as mock_search:
    ...
    async def test_knowledge_base_coverage(self):
        ...
        covered = sum(
            1 for topic in expected_topics if topic in str(topics).lower()
        )
        assert covered >= len(expected_topics) // 2

    @pytest.mark.unit
    def test_rag_relevance_filtering(self):
    ...
    async def test_medical_knowledge_retrieval_caching(self):
        """Test that search returns consistent results for same query"""
        from app.services.vector_store_simple import search_medical_knowledge
